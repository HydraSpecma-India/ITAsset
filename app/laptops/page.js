...
export default function LaptopsPage() {
  ...
  function updateProposalItem(index, field, val) {
    setProposalItems((prev) =>
      prev.map((item, idx) => {
        ...
        if (field === "laptop_category" || field === "employee_name") {
  ...
  function handleQuickDraftPrint(e) {
    ...
    setProposalPrintData({
      ...
      proposalNo: `HS/IT/LAP-PROP/${year}/DRAFT`,
    });
  ...
  async function handleSaveOfficialProposal(e) {
    ...
    try {
      ...
      const year = new Date().getFullYear();
      const propNo = `HS/IT/LAP-PROP/${year}/${String(propSeq).padStart(3, "0")}`;
  ...
  function handleRetrieveProposal(propRecord) {
    setProposalTitle(propRecord.title || "Laptop Allocation & Purchase Proposal");
  ...
  return (
    ...
    >
      ...
      {/* Modal for Creating / Editing Multi-Person Laptop Allocation Proposal */}
      {proposalModalOpen && (
        <Modal
          title={retrievedVersion ? `🔄 Edit Proposal & Submit New Version (Loaded from ${retrievedVersion}) — ${proposalItems.length} Employee(s)` : `📄 Create Laptop Allocation Proposal — ${proposalItems.length} Employee(s)`}
