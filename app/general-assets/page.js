...
export default function GeneralAssetsPage() {
  ...
  function updateProposalItem(index, field, val) {
    setProposalItems((prev) =>
      prev.map((item, idx) => {
        ...
        if (field === "asset_category" || field === "employee_name") {
  ...
  function handleRetrieveProposal(propRecord) {
    setProposalTitle(propRecord.title || "General IT Asset Allocation Proposal");
  ...
  return (
    ...
    >
      ...
      {/* Modal for Creating / Editing Multi-Person General IT Asset Allocation Proposal */}
      {proposalModalOpen && (
        <Modal
          title={retrievedVersion ? `🔄 Edit Proposal & Submit New Version (Loaded from ${retrievedVersion}) — ${proposalItems.length} Employee(s)` : `📄 Create General IT Asset Proposal — ${proposalItems.length} Employee(s)`}
