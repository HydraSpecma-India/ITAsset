...
export default function GeneralAssetsPage() {
  ...
  const [proposalData, setProposalData] = useState({
    ...
    asset_category: "Dual Monitor & Docking Station (₹25k)",
    budget_amount: 25000,
    proposed_device: "Dell 27-inch 4K Monitor + Thunderbolt 4 Dock",
    justification: "Required for dual-screen productivity, remote docking and department asset policy.",
    proposal_date: todayISO(),
  });
  const [proposalPrintRow, setProposalPrintRow] = useState(null);

  function handleOpenProposal(catName = "Dual Monitor & Docking Station (₹25k)") {
    ...
    setProposalData({
      ...
      location: "Oragadam",
      asset_category: selectedCatObj ? selectedCatObj.id : catName,
      budget_amount: selectedCatObj ? selectedCatObj.budget : 25000,
      proposed_device: selectedCatObj ? `${selectedCatObj.id} Gear` : "IT Peripheral Gear",
      justification: "Required for workplace ergonomics, engineering productivity, and policy entitlement.",
      proposal_date: todayISO(),
    });
    setEmpSearchQuery("");
  ...
  function handleGenerateProposalPrint(e) {
    ...
    if (!proposalData.employee_name.trim()) {
      alert("Please enter or select Employee Name for the General Asset Proposal.");
      return;
    }
  ...
  return (
        ...
        gridMode ? (
          ...
          <>
            <button
              ...
              onClick={() => handleOpenProposal("Dual Monitor & Docking Station (₹25k)")}
    ...
    >
      ...
      {proposalModalOpen && (
        <Modal
          title={`📄 Create General IT Asset Proposal — ${proposalData.asset_category}`}
        ...
        >
          <form onSubmit={handleGenerateProposalPrint} className="stack" style={{ gap: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <Field label="Asset Category Tier *">
                <select
                  value={proposalData.asset_category}
                  onChange={(e) => {
                    const obj = GENERAL_TIERS.find((t) => t.id === e.target.value);
                    setProposalData({
                      ...proposalData,
            ...
            <Field label="Proposed Hardware & Specifications">
              <input
                type="text"
                placeholder="e.g. Dell 27 4K Monitor + Universal USB-C Docking Station"
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 8 }}>
              ...
              <button type="submit" className="btn primary">📄 Generate & Print General Asset Proposal</button>
            </div>
          </form>
        </Modal>
      )}

      {/* Printable General Asset Proposal Form */}
      {proposalPrintRow && (
        <Modal title={`📄 General IT Asset Proposal Form — ${proposalPrintRow.employee_name}`} onClose={() => setProposalPrintRow(null)}>
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12, gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ textAlign: "center", marginBottom: 24 }}>
                <h2 style={{ fontSize: 20, fontWeight: 800, textDecoration: "underline", textUnderlineOffset: 5, margin: 0, color: "#000000", textTransform: "uppercase" }}>
                  General IT Asset Allocation Proposal
                </h2>
                <div style={{ fontSize: 11, color: "#555555", marginTop: 4, fontWeight: 600 }}>
                  Ref: HS/IT/GEN-PROP/{new Date().getFullYear()} &nbsp;|&nbsp; Date: {formatDateDDMMMYYYY(proposalPrintRow.proposal_date)}
                </div>
              </div>
              ...
              <div style={{ marginBottom: 24, padding: 14, background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 6, fontSize: 13, lineHeight: 2 }}>
                <div style={{ fontWeight: 800, color: "#000000", borderBottom: "1px solid #d1d5db", paddingBottom: 4, marginBottom: 8 }}>
                  ⌨️ Proposed IT Asset Category Tier
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "170px 20px 1fr", alignItems: "center" }}>
                  <span style={{ fontWeight: 700 }}>Asset Category Tier</span><span>:</span><span style={{ fontWeight: 800, color: "#2563eb" }}>{proposalPrintRow.asset_category}</span>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "170px 20px 1fr", alignItems: "center" }}>
              ...
              <div style={{ marginBottom: 35, fontSize: 12, lineHeight: 1.6 }}>
                ...
                <p style={{ margin: 0, padding: "10px 14px", borderLeft: "3px solid #2563eb", background: "#f8fafc" }}>
                  {proposalPrintRow.justification || "Proposed as per company IT hardware policy and department equipment entitlement."}
                </p>
              </div>
            </div>
